...
const GraphView = forwardRef(function GraphView(
  ...
  const recomputeBoundsRef = useRef(null)
  ...
  useEffect(() => {
    ...
    const edgesFromLayout = (layout.edges || []).map(edge => {
      ...
      const sourceEdge = Number.isFinite(idx) ? graph.edges[idx] : null
      const predicate = sourceEdge?.predicate || edge.labels?.[0]?.text
      return {
        ...edge,
        predicate: predicate || edge.labels?.[0]?.text || '',
        predicateIri: sourceEdge?.predicateIri || '',
      }
    })
    ...
    function formatEdgeTooltipHtml(d) {
      const shortName = escapeHtml(d.predicate || '(unnamed)')
      const iri = d.predicateIri || d.predicate || ''
      const iriLine = escapeHtml(iri)
      return `
        <div style="font-weight:600;font-size:13px;">Predicate</div>
        <div style="margin-top:4px;color:#e2e8f0;">Short: ${shortName}</div>
        <div style="margin-top:4px;color:#cbd5f5;word-break:break-all;">IRI: ${iriLine}</div>
      `
    }
    ...
    const clampTransform = transform => {
      ...
      const cMinX = minPoint[0]
      const cMinY = minPoint[1]
      const cMaxX = maxPoint[0]
      const cMaxY = maxPoint[1]
      const k = transform.k

      if (!Number.isFinite(k) || k <= 0) return transform

      let tx
      let ty

      if (k >= 1) {
        const minTx = cMaxX * (1 - k)
        const maxTx = cMinX * (1 - k)
        tx = Math.min(Math.max(transform.x, minTx), maxTx)
        const minTy = cMaxY * (1 - k)
        const maxTy = cMinY * (1 - k)
        ty = Math.min(Math.max(transform.y, minTy), maxTy)
      } else {
        tx = (cMinX + cMaxX) * (1 - k) / 2
        ty = (cMinY + cMaxY) * (1 - k) / 2
      }
    ...
    const linkHitPaths = linkGroup
      ...
      .on('click', (event, d) => {
        ...
        tooltip.style('opacity', '1').html(formatEdgeTooltipHtml(d))
    ...
    svg.on('click', event => {
      ...
      if (target && target.closest && (target.closest('.node') || target.closest('.link-hit'))) return
      hideTooltip()
    })
    ...
    if (enablePhysics) {
      const simulation = d3
        ...
        .velocityDecay(0.4)
        .alpha(0.7)
        .alphaDecay(nodes.length > 80 ? 0.28 : 0.12)
      ...
      simulation.on('tick', () => {
        ...
        const pathFn = d => (useSmoothEdges ? linkPathSmooth(d) : linkPathStraight(d))
        linkPaths.attr('d', pathFn)
        linkHitPaths.attr('d', pathFn)
      })
    ...
    recomputeBoundsRef.current = () => updateViewportBounds(nodes)
