...
export default function OutputPanel({
  ...
  return (
    <div className={`rounded-2xl bg-white/90 backdrop-blur border border-slate-200 shadow-xl p-4 ${className}`}>
      ...
      <div id={containerId} className="overflow-auto border border-slate-200 rounded-xl bg-white shadow-inner" style={{ height: '85vh' }}>
        {graph ? (
          <GraphView
            ref={viewRef}
            graph={graph}
            cfg={config}
            enablePhysics={
              Boolean(config?.exploreMode) &&
              !config?.evaluationMode &&
              (graph?.nodes?.length || 0) <= 150
            }
            onMetrics={onMetrics}
          />
        ) : (
